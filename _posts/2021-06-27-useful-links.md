---
title: Useful Links
layout: post
author: hyprcub
tags: ref
---

Here you'll find ressources I've found very useful. I'll keep adding some.

# Reverse Shells
 
 - [Reverse Shell Generator](https://www.revshells.com/)
 - [PayloadsAllTheThings - Reverse Shell Cheat Sheet](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Reverse%20Shell%20Cheatsheet.md)
 - [Get Reverse-shell via Windows one-liner](https://www.hackingarticles.in/get-reverse-shell-via-windows-one-liner/)
 - [Upgrading Simple Shells to Fully Interactive TTYs](https://blog.ropnop.com/upgrading-simple-shells-to-fully-interactive-ttys/)

# Tools

- [CyberChef - The Cyber Swiss Army Knife](https://gchq.github.io/CyberChef)
- [CrackStation - Free Password Hash Cracker](https://crackstation.net/)
- [MD5Hashing.net - Hash reverse lookup, unhash, decrypt, search](https://md5hashing.net/)
- [Useful French cipher tools website](https://www.dcode.fr/)

# Exploitations

- [GTFOBins is a curated list of Unix binaries that can be used to bypass local security restrictions in misconfigured systems](https://gtfobins.github.io/)
- [Abusing Wildcards for Tar Argument Injection in root cronjob (Nix)](https://int0x33.medium.com/day-67-tar-cron-2-root-abusing-wildcards-for-tar-argument-injection-in-root-cronjob-nix-c65c59a77f5e)
- [Escape From SHELLcatraz - Breaking Out of Restricted Unix Shells](https://speakerdeck.com/knaps/escape-from-shellcatraz-breaking-out-of-restricted-unix-shells)
- [Privilege Escalation via Python Library Hijacking](https://rastating.github.io/privilege-escalation-via-python-library-hijacking/)

# Kernel Exploits

- [Precompiled Linux kernel exploits](https://github.com/lucyoa/kernel-exploits)
- [Also precompiler Linux kernel exploits](https://github.com/SecWiki/linux-kernel-exploits)
- [Precompiled Windows kernel exploits](https://github.com/SecWiki/windows-kernel-exploits)

# Cheat Sheets

- [PayloadAllTheThings - A list of useful payloads and bypass for Web Application Security and Pentest/CTF](https://github.com/swisskyrepo/PayloadsAllTheThings)
- [MySQL cheatsheet](https://devhints.io/mysql)
- [Hashcat hash type numbers](https://hashcat.net/wiki/doku.php?id=example_hashes)
- [OWASP Testing Checklist](https://wiki.owasp.org/index.php/Testing_Checklist)

# Guides

- [HackTricks by Carlos Polop](https://book.hacktricks.xyz/)
- [Hacking Articles - Raj Chandel's Blog](https://www.hackingarticles.in)
- [A Virgil's Guide to Pentest](https://virgil-cj.blogspot.com/2018/02/enumeration-is-key_6.html)
- [Basic PowerShell for Pentesters](https://book.hacktricks.xyz/windows/basic-powershell-for-pentesters)


